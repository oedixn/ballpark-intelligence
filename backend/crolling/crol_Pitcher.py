...
URLS = {
    "basic1": "https://www.koreabaseball.com/Record/Player/PitcherBasic/Basic1.aspx",
    "basic2": "https://www.koreabaseball.com/Record/Player/PitcherBasic/Basic2.aspx",
    "detail1": "https://www.koreabaseball.com/Record/Player/PitcherBasic/Detail1.aspx",
    "detail2": "https://www.koreabaseball.com/Record/Player/PitcherBasic/Detail2.aspx",  # 없거나 구조 다를 수 있음
}
...
FINAL_COLUMNS = [
    ...
    "CG", "SHO", "QS", "BSV", "TBF", "NP", "AVG", "2B", "3B", "SAC", "SF", "IBB", "WP", "BK",
    "GS", "Wgs", "Wgr", "GF", "SVO", "TS", "GDP", "GO", "AO", "GO/AO",
...
def find_target_table(soup: BeautifulSoup) -> Optional[BeautifulSoup]:
    ...
    for table in soup.find_all("table"):
        ...
        if "선수명" in text and "팀명" in text:
            score = 0
            for keyword in ["ERA", "WHIP", "QS", "GO/AO", "K/9", "OPS", "WPCT"]:
                if keyword in text:
                    score += 1
...
def crawl_pitcher_all_columns(
    ...
    basic1_rows = safe_crawl_page_group(session, URLS["basic1"], season, team_code, series, sleep_sec)
    basic2_rows = safe_crawl_page_group(session, URLS["basic2"], season, team_code, series, sleep_sec)
    detail1_rows = safe_crawl_page_group(session, URLS["detail1"], season, team_code, series, sleep_sec)
    detail2_rows = safe_crawl_page_group(session, URLS["detail2"], season, team_code, series, sleep_sec)

    merged = merge_player_rows(basic1_rows, basic2_rows, detail1_rows, detail2_rows)
...
def main():
    ...
    for year in range(2024, 2026): #시작년도, 끝년도-1
        season = str(year)
        print(f"수집 시작: season={season}, team={team_code}")

        rows = crawl_pitcher_all_columns(
            season=season,
            team_code=team_code,
            series=series,
            sleep_sec=0.7,
    ...
    save_csv(all_rows, f"pitcher_all_columns_2021_2024_{team_code}.csv")
    print("CSV 저장 완료")
