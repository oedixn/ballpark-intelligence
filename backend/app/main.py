...
def _get_league_avg() -> Optional[LeagueAverageProb]:
    try:
        c=get_conn(); cr=get_cur(c)
        cr.execute("""
            SELECT SUM(h) AS h, SUM(bb) AS bb, SUM(hr) AS hr, SUM(tbf) AS tbf
            FROM player_pitcher_stats WHERE season_year=%s AND tbf>0
        """, (SEASON,))
        r=cr.fetchone(); cr.close(); c.close()
        if not r or not r['tbf']: raise Exception()
        tbf=r['tbf']; h=r['h'] or 0; bb=r['bb'] or 0; hr=r['hr'] or 0
        single=max(0,(h-hr)*0.8); double_=max(0,(h-hr)*0.2); out=max(0,tbf-h-bb)
        return LeagueAverageProb(bb=bb/tbf,single=single/tbf,double=double_/tbf,triple=0.003,hr=hr/tbf,out=out/tbf)
    except Exception:
        return LeagueAverageProb(bb=0.085,single=0.150,double=0.040,triple=0.004,hr=0.028,out=0.693)
...
def get_players(search: Optional[str]=None):
    try:
        c=get_conn(); cr=get_cur(c); like=[f"%{search}%"]*2 if search else []
...
def get_teams():
    try:
        c=get_conn(); cr=get_cur(c); cr.execute("SELECT * FROM teams ORDER BY team_name")
...
def get_team_pitchers(team_name:str):
    try:
        ...
        cr.execute("""
            ...
            JOIN players p ON ps.player_id = p.player_id
            JOIN teams t ON ps.team_id = t.team_id
            WHERE t.team_name = %s AND ps.season_year = %s AND ps.gs >= 3
            ORDER BY ps.gs DESC LIMIT 15
        """, (team_name, SEASON))
        r=cr.fetchall(); cr.close(); c.close()
...
def _fetch_schedule(month:str) -> list:
    data=urllib.parse.urlencode({"leId":"1","srId":"0","srIdList":"0,9","seasonId":"2026","gameWeek":"","teamId":"","stadiumId":"","gameId":"","gameDay":"","gameMonth":month}).encode()
    ...
    with urllib.request.urlopen(req,timeout=10) as res: return J.loads(res.read().decode("utf-8")).get("rows",[])

def _parse_schedule_rows(raw_rows:list) -> list:
...
def get_schedule(month:Optional[str]=None):
    try:
        tm=month or f"{datetime.now().month:02d}"
        return {"month":tm,"games":_parse_schedule_rows(_fetch_schedule(tm))}
    except Exception as e: raise HTTPException(500,str(e))
...
def get_team_recent(team_name:str):
    try:
        ...
        for month in [f"{now.month-1:02d}",f"{now.month:02d}"]:
            ...
            try: raw_rows=_fetch_schedule(month)
            except: continue
            for g in _parse_schedule_rows(raw_rows):
                if not g["score_a"] or not g["score_b"]: continue
                ta,tb,sa,sb=g["team_a"],g["team_b"],g["score_a"],g["score_b"]
                wc_raw=g["result"] or ""
                if ta==team_name:
                    results.append({"date":g["date"],"opponent":tb,"score":f"{sa}:{sb}","result":"W" if f"{ta} 승" in wc_raw else "L" if f"{tb} 승" in wc_raw else "D"})
                elif tb==team_name:
                    results.append({"date":g["date"],"opponent":ta,"score":f"{sb}:{sa}","result":"W" if f"{tb} 승" in wc_raw else "L" if f"{ta} 승" in wc_raw else "D"})
        return {"team":team_name,"recent":results[-5:] if len(results)>=5 else results}
    except Exception as e: raise HTTPException(500,str(e))
...
def get_player_image(player_id:int):
    try:
        req=urllib.request.Request(f"https://www.koreabaseball.com/file/Image/Player/2026/M/{player_id}.jpg",
            headers={"User-Agent":"Mozilla/5.0","Referer":"https://www.koreabaseball.com/Player/Search.aspx","Accept":"image/*,*/*;q=0.8"})
