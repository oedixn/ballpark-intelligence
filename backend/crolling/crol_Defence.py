...
TARGET_URL = "https://www.koreabaseball.com/Record/Player/Defense/Basic.aspx"
...
FINAL_COLUMNS = [
    "순위", "선수명", "팀명", "POS", "G", "GS", "IP", "E", "PKO", "PO", "A", "DP",
    "FPCT", "PB", "SB", "CS", "CS%",
...
def apply_filters_step_by_step(
    ...
    # 포지션 (전체면 공백)
    if position != "":
        soup = change_select(session, url, soup, FIELD_NAMES["position"], position)
...
def find_target_table(soup: BeautifulSoup) -> Optional[BeautifulSoup]:
    ...
    for table in soup.find_all("table"):
        ...
        if "선수명" in text and "팀명" in text and "POS" in text and "FPCT" in text:
            score = 0
            for keyword in ["G", "GS", "IP", "E", "PO", "A", "DP", "PB", "SB", "CS", "CS%"]:
                if keyword in text:
                    score += 1
...
def crawl_defense_all_columns(
    ...
    if table is None:
        raise RuntimeError("수비 기록 테이블을 찾지 못했습니다.")

    all_rows = []
    seen = set()

    page1_rows = parse_table(table, season, team_code)
    for row in page1_rows:
        key = (row.get("player_id"), row.get("선수명"), row.get("팀명"), row.get("POS"))
        if key not in seen:
            seen.add(key)
            all_rows.append(row)
    ...
    for page_no in page_numbers:
        ...
        next_soup = go_to_page(session, TARGET_URL, soup, page_no)
    ...
    all_rows = filter_team_only(all_rows, expected_team_name)
...
def main():
    ...
    for year in range(2021, 2025): #시작년도, 끝년도-1
        season = str(year)
        print(f"수집 시작: season={season}, team={team_code}")

        rows = crawl_defense_all_columns(
            season=season,
            team_code=team_code,
            series=series,
            position="",
    ...
    save_csv(all_rows, f"defense_all_columns_2021_2024_{team_code}.csv")
    print("CSV 저장 완료")
